			...
			<div class="m-card">
				...
				<v-data-table
					...
					<template v-slot:item.title="{ item }">
						<router-link class="black--text font-weight-medium" target="_blank" :to="'/shop/' + item.store">{{ item.title }}</router-link>
					...
					<template v-slot:item.options="{ item }">
						<v-btn small icon @click.prevent="publish([item], shops.indexOf(item))">
...
<script>
	export default {
		...
		async asyncData({ app, $axios, redirect }) {
			// Check if allowed
			if (!app.$mogate.allow(app.$auth.user, 'approve', 'shops')) {
				redirect('/moderator')
			}

			let response = await $axios.get('moderator/pending/shops')
			return {
		...
		computed: {
			headers() {
				return [
		          	{ text: this.$t('t_logo'), value: 'logo', sortable: false, align: this.$vuetify.rtl ? 'right' : 'left'},
		          	{ text: this.$t('t_title'), value: 'title', sortable: false, align: this.$vuetify.rtl ? 'right' : 'left'},
		          	{ text: this.$t('t_shop_subtitle'), value: 'subtitle', sortable: false, align: 'center'},
		...
		methods: {
			getNextPage(page = 1) {
				this.loading = true
				this.$axios
				.get('moderator/pending/shops?page=' + page)
				.then(response => {
					this.selected  = []
			...
			publish: function(shops = this.selected, index = null) {
				...
				vm.loading = true
				this.$axios
				.post('moderator/shops/options/publish', {
					shops: shops
				})
				.then(response => {
					if (index !== null) {
