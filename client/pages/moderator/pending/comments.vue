			...
			<div class="m-card">
				...
				<v-data-table
					...
					<template v-slot:item.options="{ item }">
						<!-- Publish -->
						<v-btn small slot="activator" icon @click.prevent="publish([item], comments.indexOf(item))">
							<v-icon size="18px" color="grey darken-1">mdi-check</v-icon>
						</v-btn>

						<!-- Read -->
						<v-btn small slot="activator" icon @click.prevent="readComment(item.comment)">
							<v-icon size="18px" color="grey darken-1">mdi-eye</v-icon>
						</v-btn>
					</template>
				</v-data-table>
			</div>
...
<script>
	export default {
		...
		async asyncData({ app, $axios, redirect }) {
			// Check if allowed
			if (!app.$mogate.allow(app.$auth.user, 'approve', 'comments')) {
				redirect('/moderator')
			}

			let response = await $axios.get('moderator/pending/comments')
			return {
		...
		methods: {
			getNextPage(page = 1) {
				this.loading = true
				this.$axios
				.get('moderator/pending/comments?page=' + page)
				.then(response => {
					this.selected     = []
