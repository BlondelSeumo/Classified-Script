			...
			<v-card
				...
				<v-card
					...
					<v-card-title class="pa-2 white">
						...
						<v-btn icon v-if="$mogate.allow($auth.user, 'edit', 'shops')" text :to="'/moderator/shops/edit/' + store.details.shop.store" color="#4C4A48">
							<v-icon>mdi-square-edit-outline</v-icon>
						</v-btn>
						<v-btn icon text @click="remove([store.details.shop], store.details.index)" color="#4C4A48" v-if="$mogate.allow($auth.user, 'delete', 'shops') && store.details.shop.deleted_at === null">
							<v-icon>mdi-delete</v-icon>
						</v-btn>
						<v-btn icon text @click="restore([store.details.shop], store.details.index)" color="#4C4A48" v-if="$mogate.allow($auth.user, 'delete', 'shops') && store.details.shop.deleted_at !== null">
