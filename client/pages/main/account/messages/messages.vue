		...
		<v-dialog v-model="dialogs.reply" max-width="500px" persistent>
			<v-card class="pa-2">
				...
				<v-card-actions class="pa-2">
					<v-spacer></v-spacer>
					<v-btn color="#2e3131" text @click="send()" :disabled="!form.message">{{ $t('t_reply') }}</v-btn>
				</v-card-actions>
			</v-card>
		</v-dialog>
						...
						<v-card class="m-card" flat>
					        ...
					        <v-data-table
								...
								<template v-slot:item.message="{ item }">
									<v-btn depressed dark color="grey darken-3" text icon @click="read(item, messages.indexOf(item))">
