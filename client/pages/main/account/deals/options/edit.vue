		...
		<v-content>
      		<v-container grid-list-xl fluid>
      			<v-layout wrap fill-height>
					...
					<v-flex xs3>
                        <client-only>
                            <div v-sticky="{zIndex: 3, stickyTop: 15}">
                                <div>
                                    <file-pond
                                        name="test"
                                        ref="pond"
...
<script>
    ...
    export default {
        ...
        async asyncData({ $axios, params, query }) {
			let response = await $axios.post('account/deals/options/edit', { id: params.id, package: query.package })
			return {
				form: {
        ...
        head() {
			return {
	  			title: this.$t('t_edit_deal_title', {title: this.deal.title}),
        ...
        methods: {
            update() {
                try {
					...
					this.$axios
					.post('account/deals/options/update',
					  	data,
					  	{
						    headers: {
					...
					.then(response => {
						...
						this.$toasted.show(this.$t('t_toasted_your_deal_edited'), {
							icon: 'done_all',
							className: this.$vuetify.rtl ? 'toasted-rtl' : ''
						})
