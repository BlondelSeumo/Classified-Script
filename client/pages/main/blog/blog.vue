		...
		<v-content>
      		<v-container grid-list-xl fluid>
      			<v-layout wrap>
					...
					<v-flex xs3 v-for="(article, index) in articles" :key="index">
						<v-card class="m-card mb-3" flat>
							...
							<v-card-text style="height: 100px" class="px-3 py-4" v-html="article.content.substring(0, 150)"></v-card-text>
							<v-card-actions class="px-3 py-4">
								<v-btn depressed fab small class="red--text" :href="'http://reddit.com/submit?url=' + $home('blog/' + article.slug) + '&title=' + article.title" target="_blank">
									<v-icon class="mdi mdi-reddit"></v-icon>
								</v-btn>
								<v-btn depressed fab small class="blue--text text--darken-4" :href="'https://www.facebook.com/sharer/sharer.php?u=' + $home('blog/' + article.slug)" target="_blank">
									<v-icon class="mdi mdi-facebook"></v-icon>
								</v-btn>
								<v-btn depressed fab small class="light-blue--text" :href="'https://twitter.com/home?status=' + $home('blog/' + article.slug)" target="_blank">
