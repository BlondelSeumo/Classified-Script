		...
		<v-dialog width="400" v-model="dialog.share" persistent>
			<v-card>
				...
				<v-card-text class="text-center">
					<qrcode :value="$home('listing/' + listing.unique_slug)" :options="{ width: 100, margin: 0, color: {dark: '#4b575f'} }"></qrcode>
					...
					<v-flex xs12 v-else-if="$auth.loggedIn && $isModerator() && listing.isPending && $mogate.allow($auth.user, 'approve', 'deals')">
      				...
      				<v-flex xs8>
      					<v-toolbar
					      	...
					      	dense
					      	class="pa-0 mb-4"
					      	style="-webkit-box-shadow:0 0 0 1px rgba(63,63,68,.05),0 1px 3px 0 rgba(63,63,68,.15)!important;box-shadow:0 0 0 1px rgba(63,63,68,.05),0 1px 3px 0 rgba(63,63,68,.15)!important;"
					    >
					      	<v-breadcrumbs :items="breadcrumbs">
					          	<v-icon slot="divider" v-if="$vuetify.rtl">mdi-chevron-left</v-icon>
					          	<v-icon slot="divider" v-else>mdi-chevron-right</v-icon>
					      	</v-breadcrumbs>
					      	<v-spacer></v-spacer>
					      	<span style="font-size:12px;color:#9f9f9f;font-weight:500;">{{ $ago(listing.created_at) }}</span>
					    </v-toolbar>
						<v-card class="mx-auto m-card">
							<v-carousel
								...
								continuous
								cycle
								v-if="listing.photosNumber !== 0"
								>
								<v-carousel-item
									v-for="(i, index) in listing.photosNumber"
									:key="index"
									:src="sliderPicture(i)"
									>
								</v-carousel-item>
							...
							<v-carousel
								...
								>
								<v-carousel-item
									:src="$homeApi('storage/app/uploads/default/classifieds/no-image.png')"
									>
								</v-carousel-item>
							</v-carousel>
							<v-list two-line>
								<v-list-item>
									...
									<v-list-item-content>
										<v-list-item-title class="headline pb-2">{{ listing.user.username }}</v-list-item-title>
										<v-list-item-subtitle>{{ $t('t_member_since', {date: $dateSince(listing.user.created_at)}) }}</v-list-item-subtitle>
									</v-list-item-content>
									<v-list-item-action class="mx-0">
										<div class="text-left follow-btn">

											<!-- Message -->
											<v-tooltip top v-if="$auth.loggedIn && $auth.user.id !== listing.user.id && $megate.allow($auth.user, 'send', 'messages')">
												<template v-slot:activator="{ on }">
													<v-btn @click="dialog.contact = true" v-on="on" :class="$vuetify.rtl ? 'mr-2' : 'ml-2'" medium text icon color="blue-grey darken-4">
														<v-icon small>mdi-message</v-icon>
													</v-btn>
												</template>
												<span>{{ $t('t_contact_me') }}</span>
											</v-tooltip>

											<!-- Phone number -->
											<v-tooltip top v-if="listing.user.phone">
												<template v-slot:activator="{ on }">
													<v-btn v-on="on" @click="dialog.phone = true" :class="$vuetify.rtl ? 'mr-2' : 'ml-2'" medium text icon color="blue-grey darken-4">
											...
											<v-tooltip top>
												<template v-slot:activator="{ on }">
													<v-btn v-on="on" @click="dialog.share = true" :class="$vuetify.rtl ? 'mr-2' : 'ml-2'" medium text icon color="blue-grey darken-4">
														<v-icon size="20px">mdi-open-in-app</v-icon>
													</v-btn>
												</template>
												<span>{{ $t('t_share') }}</span>
											</v-tooltip>
											...
											<!-- Report -->
											<v-tooltip top v-if="$megate.allow($auth.user, 'deals', 'reports')">
												<template v-slot:activator="{ on }">
													<v-btn v-on="on" @click.prevent="reportDeal" :class="$vuetify.rtl ? 'mr-2' : 'ml-2'" medium text icon color="blue-grey darken-4">
														<v-icon size="20px">mdi-alert-octagon</v-icon>
													</v-btn>
												</template>
												<span>{{ $t('t_report') }}</span>
											</v-tooltip>

											<!-- Send offer -->
											<v-tooltip top v-if="$megate.allow($auth.user, 'make', 'offers')">
												<template v-slot:activator="{ on }">
													<v-btn v-on="on" @click="dialog.offer = true" :class="$vuetify.rtl ? 'mr-2' : 'ml-2'" medium text icon color="blue-grey darken-4">
														<v-icon size="18px">mdi-tag</v-icon>
													</v-btn>
												</template>
												<span>{{ $t('t_send_an_offer') }}</span>
											</v-tooltip>

											<!-- Youtube video -->
											<v-tooltip top v-if="listing.video">
												<template v-slot:activator="{ on }">
													<v-btn v-on="on" @click="dialog.video = !dialog.video" :class="$vuetify.rtl ? 'mr-2' : 'ml-2'" medium text icon color="blue-grey darken-4">
														<v-icon size="18px">mdi-youtube</v-icon>
													</v-btn>
												</template>
												<span>{{ $t('t_watch_video') }}</span>
											</v-tooltip>

											<!-- Action -->
											<v-tooltip top v-if="$auth.loggedIn && listing.user_id === $auth.user.id">
												<template v-slot:activator="{ on }">
													<v-btn v-on="on" nuxt :to="`/account/deals/edit/${listing.unique_id}`" :class="$vuetify.rtl ? 'mr-2' : 'ml-2'" medium text icon color="blue-grey darken-4">
														<v-icon size="18px">mdi-lead-pencil</v-icon>
													</v-btn>
												</template>
												<span>{{ $t('t_edit_deal') }}</span>
											</v-tooltip>
										</div>
									</v-list-item-action>
								</v-list-item>
							...
							<v-divider class="mb-3" style="border-color: rgba(0, 0, 0, 0.05);"></v-divider>

							<!-- Youtube video -->
							<v-card-text v-if="listing.video && dialog.video">
								<iframe width="100%" height="400" :src="'https://www.youtube.com/embed/' + $getYoutubeId(listing.video)" style="display: block;border-width: 0"></iframe>
							</v-card-text>
						  	...
						  	<v-card-text class="px-4 pt-4" v-if="fields.length !== 0">
									...
									<v-container grid-list-xl fluid fill-height class="my-3 mx-0 pa-0">
										<v-layout wrap>
											...
											<template v-for="(item, index) in fields">
												<v-flex xs4 :key="index" v-if="item.field.type === 'dropdown' || item.field.type === 'radio'">
													<v-list-item style="background-color: #fafafa">
														...
														<v-list-item-content>
															...
															<v-list-item-subtitle style="color: #171717">{{ item.options }}</v-list-item-subtitle>
														</v-list-item-content>
													</v-list-item>
												</v-flex>
											</template>
								...
								<v-container grid-list-xl fluid fill-height class="my-3 mx-0 pa-0">
									<v-layout wrap>
										...
										<template v-for="(item, index) in fields">
											<v-flex xs4 :key="index" v-if="item.field.type === 'checkbox'">
							...
							<v-card-text class="py-4 " v-if="listing.description">
								<no-ssr>
									<truncate action-class="text-center caption text-uppercase" :clamp="$t('t_show_more')" :length="280" :less="$t('t_show_less')" type="html" :text="listing.description" v-if="listing.description"></truncate>
								</no-ssr>
							</v-card-text>
      					</v-card>

      					<!-- Advertisement -->
      					<v-card class="advertisement mt-4 mb-4 m-card pa-0 text-center" v-if="advertisements.top_related && $megate.allow($auth.user, 'advertisements', 'see')" v-html="advertisements.top_related"></v-card>

      					<!-- Related deals -->
      					<v-layout wrap>
	      					<div v-swiper:mySwiper="swiperOption" :dir="$vuetify.rtl ? 'rtl' : ''">
							    <div class="swiper-wrapper">
							      	<div class="swiper-slide" v-for="(deal, index) in related" :key="index">
							        	<v-flex xs12 class="mb-5">
									        <v-card class="m-card ad-box" text>
												<v-img cover height="150px" :lazy-src="$homeApi('public/images/default/lazy.jpg')" :src="preview(deal)">
      					...
      					<!-- Advertisement -->
      					<v-card class="advertisement mt-4 m-card pa-0 text-center" v-if="advertisements.bottom_related && $megate.allow($auth.user, 'advertisements', 'see')" v-html="advertisements.bottom_related"></v-card>

						<!-- comments -->
      					<v-card class="m-card mt-4 py-5" text>
					        <div class="py-4">
					        	<ul id="comments" class="px-5">
									...
									<li v-for="(comment, index) in comments[0]" :key="index">
									    ...
									    <v-divider class="mb-3 mt-5" color="#f3f3f3" v-if="index != Object.keys(comments[0]).length - 1"></v-divider> 
									</li>
									<client-only>
										<infinite-loading spinner="spiral" @distance="1" @infinite="loadComments" force-use-infinite-wrapper>
											<span slot="no-more"></span>
											<span slot="no-results"></span>
											<v-alert v-if="comments.length === 0" slot="no-results" text dense color="teal" :border="$vuetify.rtl ? 'left' : 'right'">
												{{ $t('t_no_comment_now') }}
											</v-alert>
										</infinite-loading>
									</client-only>
      				...
      				<v-flex xs4>
      					...
      					<v-card class="m-card mb-4" text>
      						<v-card-text class="pa-5 text-center">
								<strong class="display-1 font-weight-black" v-if="listing.price !== null && listing.currency !== null && listing.locale !== null">{{ $getCurrency(listing.price, listing.currency, listing.locale) }}</strong>
								<strong class="display-1 font-weight-black" v-else>{{ $t('t_n_a') }}</strong>
      						</v-card-text>
      					</v-card>

      					<!-- Map -->
      					<v-card class="m-card mb-4 pa-0" text v-if="listing.latitude && listing.longitude">
							<iframe
      					...
      					<!-- Advertisement -->
      					<v-card class="advertisement mt-4 mb-4 m-card pa-0 text-center" v-if="advertisements.sidebar && $megate.allow($auth.user, 'advertisements', 'see')" v-html="advertisements.sidebar"></v-card>
      				</v-flex>
...
<style scoped>
	...
	.related-swiper .swiper-button-next, .swiper-container-rtl .swiper-button-prev {
		right: 0;
	}
	.related-swiper .swiper-button-prev, .swiper-container-rtl .swiper-button-next {
		left: 0
	}
</style>
