		...
		<v-container>
			<v-card class="m-card" flat>
				<v-card-title primary-title class="pa-4">
		          <div>
		            <div class="title">{{ $t('t_create_field') }}</div>
...
<script>
	...
	export default {
		...
		async asyncData({ app, $axios, redirect }) {
			// Check if allowed
			if (!app.$adgate.allow(app.$auth.user, 'create', 'fields')) {
				redirect('/administrator')
			}

			let response = await $axios.post('administrator/ajax/fetch/categories/all')
			return {
		...
		methods: {
			create: function() {
				// Not allowed?
				if (!this.$adgate.allow(this.$auth.user, 'create', 'fields')) {
				...
				this.loading = true
				this.$axios
				.post('administrator/fields/options/create', {
					name: this.form.name,
					slug: this.form.slug,
					type: this.form.type,
				...
				.then(response => {
					...
					this.$toasted.show(this.$t('t_toasted_field_created'), {
						icon: 'done_all',
