		...
		<v-container>
			<v-card class="m-card" flat>
				<v-card-title primary-title class="pa-4">
		          <div>
		            <div class="title">{{ $t('t_edit_page') }}</div>
		            <span class="card-description">{{ $t('t_edit_page_para') }}</span>
		          </div>
		        </v-card-title>
...
<script>
	...
	export default {
		...
		async asyncData({ app, $axios, params, redirect }) {
			// Check if allowed
			if (!app.$adgate.allow(app.$auth.user, 'edit', 'pages')) {
				redirect('/administrator')
			}

			let response = await $axios.post('administrator/pages/options/edit', { slug: params.slug })
			return {
				columns: {
					first: response.data.config.first,
					second: response.data.config.second,
					third: response.data.config.third,
					fourth: response.data.config.fourth
				},
				form: {
					...
					is_link   : response.data.page.isLink,
					link      : response.data.page.link,
		...
		methods: {
			update: function() {
				// Not allowed?
				if (!this.$adgate.allow(this.$auth.user, 'edit', 'pages')) {
				...
				this.loading = true
				this.$axios
				.post('administrator/pages/options/update',{
					id: this.form.id,
				...
				.then(response => {
					this.errors   = []
					this.$toasted.show(this.$t('t_toasted_page_updated'), {
						icon: 'done_all',
