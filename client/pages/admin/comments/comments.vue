		...
		<v-container>
			<div class="m-card">
				<v-toolbar color="white" class="elevation-0">
					...
					<!-- Publish -->
					<v-fade-transition v-if="$adgate.allow($auth.user, 'approve', 'comments')">
						...
						<v-list two-line class="transparent">
							<v-list-item :to="`/listing/${item.deal.unique_slug}`" target="_blank">
								...
								<v-list-item-content class="table-wrap-title">
									<v-list-item-title class="table-wrap-title" v-html="title(item)"></v-list-item-title>
									<v-list-item-subtitle class="pb-1 font-weight-regular d-block caption" v-if="item.isDeal">Deals</v-list-item-subtitle>
									<v-list-item-subtitle class="pb-1 font-weight-regular d-block caption" v-if="item.isArticle">Articles</v-list-item-subtitle>
								</v-list-item-content>
							</v-list-item>
						</v-list>
					</template>
					...
					<template v-slot:item.options="{ item }">
						<v-menu bottom :left="$vuetify.rtl ? false : true" :right="$vuetify.rtl ? true : false" origin="center center" max-height="300px">
							...
							<v-list dense>
								...
								<!-- Read -->
								<v-list-item @click="read(item.comment)" v-if="$adgate.allow($auth.user, 'access', 'comments')">
									<v-list-item-action>
								...
								<!-- Publish -->
								<v-list-item @click="publish([item], comments.indexOf(item))" v-if="(item.isPending || item.isSpam) && $adgate.allow($auth.user, 'approve', 'comments')">
									<v-list-item-action>
										<v-icon>mdi-check-all</v-icon>
									</v-list-item-action>
									<v-list-item-content>
								...
								<!-- Edit -->
								<v-list-item @click="edit(item, comments.indexOf(item))" v-if="$adgate.allow($auth.user, 'edit', 'comments')">
									<v-list-item-action>
								...
								<!-- Delete -->
								<v-list-item @click="remove([item], comments.indexOf(item))" v-if="item.deleted_at === null && $adgate.allow($auth.user, 'delete', 'comments')">
									<v-list-item-action>
										<v-icon>mdi-delete</v-icon>
									</v-list-item-action>
									<v-list-item-content>
								...
								<!-- Restore -->
								<v-list-item @click="restore([item], comments.indexOf(item))" v-if="item.deleted_at !== null && $adgate.allow($auth.user, 'delete', 'comments')">
									<v-list-item-action>
										<v-icon>mdi-delete-restore</v-icon>
									</v-list-item-action>
									<v-list-item-content>
								...
								<!-- Spam -->
								<v-list-item @click="spam([item], comments.indexOf(item))" v-if="$adgate.allow($auth.user, 'delete', 'comments')">
									<v-list-item-action>
										<v-icon>mdi-alert-octagon</v-icon>
									</v-list-item-action>
									<v-list-item-content>
...
<script>
	export default {
		...
		async asyncData({ app, $axios, redirect }) {
			...
			let response = await $axios.get('administrator/comments')
			return {
		...
		methods: {
			getNextPage(page = 1) {
				this.loading = true
				this.$axios
				.get('administrator/comments?page=' + page)
				.then(response => {
					this.selected     = []
		   	...
		   	publish: function(comments = this.selected, index = null) {
				// Not allowed?
				if (!this.$adgate.allow(this.$auth.user, 'approve', 'comments')) {
					this.$router.push('/administrator')	
					return
				}
		   	...
		   	remove: function(comments = this.selected, index = null) {
				...
				vm.loading = true
				this.$axios
				.post('administrator/comments/options/delete', {
					comments: comments
				})
				.then(response => {
					if (index !== null) {
		   	...
		   	restore: function(comments = this.selected, index = null) {
				...
				vm.loading = true
				this.$axios
				.post('administrator/comments/options/restore', {
					comments: comments
				})
				.then(response => {
					if (index !== null) {
		   	...
		   	spam: function(comments = this.selected, index = null) {
				...
				vm.loading = true
				this.$axios
				.post('administrator/comments/options/spam', {
					comments: comments
				})
				.then(response => {
					if (index !== null) {
