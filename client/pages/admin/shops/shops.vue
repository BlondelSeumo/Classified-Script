<template>
	<v-app id="inspire">
		...
		<!-- Delete dialog -->
		<v-dialog v-model="dialogs.delete" max-width="400" persistent v-if="$adgate.allow($auth.user, 'delete', 'shops')">
			<v-card class="py-2">
				<v-card-text>
				...
				<v-card-actions>
					...
					<v-btn color="error" text @click="remove()">
						{{ selected.length > 1 ? this.$t('t_delete_shops') : this.$t('t_delete_shop') }}
					</v-btn>
				</v-card-actions>
			</v-card>
		</v-dialog>
		...
		<!-- Details dialog -->
		<v-dialog v-model="dialogs.details" max-width="600" persistent v-if="Object.keys(store.details.shop).length !== 0 && $adgate.allow($auth.user, 'access', 'shops')">
			<v-card
				...
				<v-card
					...
					flat
					>
					<v-card-title class="pa-2 white">
						<v-avatar>
							<v-img :src="logo(store.details.shop.logo)"></v-img>
						</v-avatar>
						<v-spacer></v-spacer>
						<v-btn icon text :to="'/shop/' + store.details.shop.store" target="_blank" color="#4C4A48">
							<v-icon>mdi-open-in-new</v-icon>
						</v-btn>
						<v-btn icon text :to="'/administrator/shops/edit/' + store.details.shop.store" color="#4C4A48" v-if="$adgate.allow($auth.user, 'edit', 'shops')">
							<v-icon>mdi-square-edit-outline</v-icon>
						</v-btn>
						<v-btn icon text @click="remove([store.details.shop], store.details.index)" color="#4C4A48" v-if="store.details.shop.deleted_at === null && $adgate.allow($auth.user, 'delete', 'shops')">
							<v-icon>mdi-delete</v-icon>
						</v-btn>
						<v-btn icon text @click="restore([store.details.shop], store.details.index)" color="#4C4A48" v-if="store.details.shop.deleted_at === null && $adgate.allow($auth.user, 'delete', 'shops')">
							<v-icon>mdi-delete-restore</v-icon>
						</v-btn>
						<v-btn icon text @click="dialogs.details = false" color="#4C4A48">
		...
		<v-container>
			<div class="m-card">
				<v-toolbar color="white" flat>
					...
					<!-- Activate -->
					<v-fade-transition v-if="$adgate.allow($auth.user, 'approve', 'shops')">
				    ...
				    <!-- Delete -->
			        <v-fade-transition v-if="$adgate.allow($auth.user, 'delete', 'shops')">
				        <v-tooltip top v-if="selected.length > 0">
							<template v-slot:activator="{ on }">
					...
					<!-- Create -->
			        <v-tooltip top v-if="$adgate.allow($auth.user, 'access', 'shops')">
						<template v-slot:activator="{ on }">
							<v-btn to="/administrator/shops/create" v-on="on" text icon>
								<v-icon>add</v-icon>
							</v-btn>
						</template>
						...
						<template v-slot:item.options="{ item }">
							<v-menu bottom :left="$vuetify.rtl ? false : true" :right="$vuetify.rtl ? true : false" origin="center center" max-height="300px">
								...
								<v-list dense>
									...
									<!-- Details -->
									<v-list-item @click="details(item, shops.indexOf(item))" v-if="$adgate.allow($auth.user, 'access', 'shops')">
										<v-list-item-action>
									...
									<!-- Activate -->
									<v-list-item v-if="(item.isPending || item.isExpired) && $adgate.allow($auth.user, 'approve', 'shops')" @click="activate([item], shops.indexOf(item))">
										<v-list-item-action>
											<v-icon>mdi-check-all</v-icon>
										</v-list-item-action>
										<v-list-item-content>
									...
									<!-- Edit -->
									<v-list-item :to="'/administrator/shops/edit/' + item.store" v-if="$adgate.allow($auth.user, 'edit', 'shops')">
										<v-list-item-action>
											<v-icon>mdi-square-edit-outline</v-icon>
										</v-list-item-action>
										<v-list-item-content>
									...
									<!-- Delete -->
									<v-list-item v-if="item.deleted_at === null && $adgate.allow($auth.user, 'delete', 'shops')" @click="remove([item], shops.indexOf(item))">
										<v-list-item-action>
											<v-icon>mdi-delete</v-icon>
										</v-list-item-action>
										<v-list-item-content>
									...
									<!-- Restore -->
									<v-list-item v-if="item.deleted_at !== null && $adgate.allow($auth.user, 'delete', 'shops')" @click="restore([item], shops.indexOf(item))">
										<v-list-item-action>
											<v-icon>mdi-delete-restore</v-icon>
										</v-list-item-action>
										<v-list-item-content>
...
<script>
	export default {
		...
		async asyncData({ app, $axios, redirect }) {
			// Check if allowed
			if (!app.$adgate.allow(app.$auth.user, 'access', 'shops')) {
				redirect('/administrator')
			}

			let response = await $axios.get('administrator/shops')
			return {
		...
		methods: {
			getNextPage(page = 1) {
				this.loading = true
				this.$axios
				.get('administrator/shops?page=' + page)
				.then(response => {
					this.selected  = []
			...
			activate: function(shops = this.selected, index = null) {
				// Not allowed?
				if (!this.$adgate.allow(this.$auth.user, 'approve', 'shops')) {
				...
				vm.loading = true
				this.$axios
				.post('administrator/shops/options/activate', {
					shops: shops
				})
				.then(response => {
					if (index !== null) {
			...
			remove: function(shops = this.selected, index = null) {
				// Not allowed?
				if (!this.$adgate.allow(this.$auth.user, 'delete', 'shops')) {
					this.$router.push('/administrator')	
					return
				}
				...
				vm.loading = true
				this.$axios
				.post('administrator/shops/options/delete', {
					shops: shops
				})
				.then(response => {
					if (index !== null) {
			...
			restore: function(shops = this.selected, index = null) {
				...
				vm.loading = true
				this.$axios
				.post('administrator/shops/options/restore', {
					shops: shops
				})
				.then(response => {
					if (index !== null) {
...
<style>
	.product-link .theme--light.v-list .v-list__group__header:hover, .theme--light.v-list .v-list__tile--highlighted, .theme--light.v-list .v-list__tile--link:hover {
