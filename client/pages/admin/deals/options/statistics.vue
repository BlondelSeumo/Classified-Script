									...
									<v-card-actions>
										<v-list-item class="grow">
											...
											<v-layout
												...
												<span class="subheading mr-2">{{ listing.visist_count | numeralFormat }}</span>
											</v-layout>
										</v-list-item>
									</v-card-actions>
								</v-flex>
						...
						<div class="m-card">
							...
							<v-data-table
								...
								<template v-slot:item.ip="{ item }">
									<a :href="'https://extreme-ip-lookup.com/' + item.ip" target="_blank" class="table-wrap-title">{{ item.ip }}</a>
									<small class="pb-1 font-weight-regular text-uppercase d-block">{{ item.provider }}</small>
								</template>
...
<script>
	export default {
		...
		async asyncData({ app, $axios, params, redirect }) {
			// Check if allowed
			if (!app.$adgate.allow(app.$auth.user, 'statistics', 'deals')) {
				redirect('/administrator')
			}

			let response = await $axios.post('administrator/deals/options/statistics', { id: params.id })
			return {
		...
		data: function() {
			return {
				...
				headers: [
		          	...
		          	{ text: this.$t('t_ip'), value: 'ip', sortable: false, align: this.$vuetify.rtl ? 'right' : 'left'},
		...
		methods: {
			...
			getMoreStatistics(page = 1) {
				this.loading = true
				this.$axios
				.get('administrator/deals/options/statistics/more/' +  this.listing.unique_id  + '?page=' + page)
				.then(response => {
