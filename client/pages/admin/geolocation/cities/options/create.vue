		...
		<v-container>
			<v-card class="m-card" flat>
				<v-card-title primary-title class="pa-4">
		          <div>
		            <div class="title">{{ $t('t_create_city') }}</div>
		            <span class="card-description">{{ $t('t_create_city_para') }}</span>
		          </div>
		        </v-card-title>
...
<script>
	export default {
		...
		async asyncData({ app, $axios, redirect }) {
			...
			let response = await $axios.get('administrator/geolocation/cities/options/create')
			return {
		...
		methods: {
			create: function() {
				...
				this.loading  = true
				this.$axios
				.post('administrator/geolocation/cities/options/create',{
					name: this.form.name,
				...
				.then(response => {
					...
					this.$toasted.show(this.$t('t_toasted_city_created'), {
						icon: 'done_all',
