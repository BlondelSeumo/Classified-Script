		...
		<v-dialog v-model="dialogs.create" max-width="400" v-if="$owgate.allow($auth.user, 'create', 'roles')">
			<v-card tile>
				...
				<v-card-text class="text-center py-5">
					...
					<!-- Owner -->
					<v-btn block depressed color="#bdc3c7" class="mb-3" to="/administrator/roles/options/create/owner">
						{{ $t('t_create_owner_role') }}
					</v-btn>

					<!-- Administrator -->
					<v-btn block depressed color="#bdc3c7" class="mb-3" to="/administrator/roles/options/create/administrator">
						{{ $t('t_create_administrator_role') }}
					</v-btn>

					<!-- Moderator -->
					<v-btn block depressed color="#bdc3c7" class="mb-3" to="/administrator/roles/options/create/moderator">
						{{ $t('t_create_moderator_role') }}
					</v-btn>

					<!-- Member -->
					<v-btn block depressed color="#bdc3c7" to="/administrator/roles/options/create/member">
			...
			<div class="m-card">
				<v-toolbar color="white" flat>
			        ...
			        <v-tooltip top v-if="$owgate.allow($auth.user, 'create', 'roles')">
						<template v-slot:activator="{ on }">
							<v-btn v-on="on" text icon color="grey darken-4" @click="dialogs.create = true">
								<v-icon>add</v-icon>
							</v-btn>
						</template>
				...
				<v-data-table
					...
					<template v-slot:item.permissions="{ item }">
						<v-tooltip top>
							<template v-slot:activator="{ on }">
								<v-btn icon text color="grey darken-3" v-on="on" @click="showPermissions(item)">
