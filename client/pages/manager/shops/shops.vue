			...
			<v-card
				...
				<v-card
					...
					<v-card-title class="pa-2 white">
						...
						<v-spacer></v-spacer>
						<v-btn icon flat :to="'/shop/' + store.details.shop.store" target="_blank" color="#4C4A48">
							<v-icon>mdi-open-in-new</v-icon>
						</v-btn>
						<v-btn icon flat :to="'/manager/shops/edit/' + store.details.shop.store" color="#4C4A48">
							<v-icon>mdi-square-edit-outline</v-icon>
						</v-btn>
						<v-btn icon flat @click="remove([store.details.shop], store.details.index)" color="#4C4A48" v-if="store.details.shop.deleted_at === null">
							<v-icon>mdi-delete</v-icon>
						</v-btn>
						<v-btn icon flat @click="dialogs.details = false" color="#4C4A48">
			        ...
			        <v-fade-transition>
				        <v-tooltip top v-if="selected.length > 0">
							<template v-slot:activator="{ on }">
								<v-btn v-on="on" flat icon color="grey darken-3" @click="dialogs.delete = true">
									<v-icon>mdi-delete</v-icon>
								</v-btn>
							</template>
