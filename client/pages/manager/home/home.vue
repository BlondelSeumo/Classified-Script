			...
			<v-layout wrap>
                ...
                <template v-for="(item, index) in items">
                    <v-flex xs2 :key="index">
                        <v-card :color="item.color" dark class="m-card no-border-radius">
                            <v-card class="text-center py-3" :color="item.colorHighlight" flat>
                                <v-icon size="30px"> {{item.icon}} </v-icon>
                            </v-card>
                            <v-card-text class="text-center px-0 py-3">
                                <div class="font-weight-bold text-uppercase caption pb-3">{{item.title}}</div>
...
<script>
    export default {
        ...
        async asyncData({ app, $axios }) {
            ...
            return {
                items: [ 
                    {color: "light-blue", colorHighlight: "#50C7FE", title: app.i18n.t('t_total_deals'), icon: "mdi-image-multiple", value: response.data.deals_count},
                    {color: "light-blue", colorHighlight: "#50C7FE", title: app.i18n.t('t_total_shops'), icon: "mdi-store", value: response.data.shops_count},
                    {color: "light-blue", colorHighlight: "#50C7FE", title: app.i18n.t('t_total_followers'), icon: "mdi-account-group", value: response.data.followers_count},
                    {color: "light-blue", colorHighlight: "#50C7FE", title: app.i18n.t('t_total_messages'), icon: "mdi-message", value: response.data.messages_count},
                    {color: "light-blue", colorHighlight: "#50C7FE", title: app.i18n.t('t_total_offers'), icon: "mdi-tag", value: response.data.offers_count},
                    {color: "light-blue", colorHighlight: "#50C7FE", title: app.i18n.t('t_total_visits'), icon: "mdi-finance", value: response.data.visits_count},
                ],
