		...
		<v-container>
			<div class="m-card">
				<v-toolbar color="white" class="elevation-0">
			        ...
			        <v-tooltip top>
						<template v-slot:activator="{ on }">
							<v-btn to="/manager/deals/create" v-on="on" text icon>
								<v-icon>add</v-icon>
							</v-btn>
						</template>
				...
				<v-data-table
					...
					<template v-slot:item.deal="{ item }">
						<v-list two-line class="transparent">
							<v-list-item :to="`/listing/${item.unique_slug}`" target="_blank" nuxt :ripple="false">
								<v-list-item-avatar>
					...
					<template v-slot:item.options="{ item }">
						<v-menu bottom :left="$vuetify.rtl ? false : true" :right="$vuetify.rtl ? true : false" origin="center center" max-height="300px">
							...
							<v-list dense>
								...
								<!-- Statistics -->
								<v-list-item :to="'/manager/deals/statistics/' + item.unique_id">
									<v-list-item-action>
								...
								<!-- Edit -->
								<v-list-item :to="'/manager/deals/edit/' + item.unique_id">
									<v-list-item-action>
...
<script>
    export default {
		...
		async asyncData({ app, $axios, redirect }) {
			let response = await $axios.get('manager/deals')
			return {
		...
		methods: {
			...
			archive: function(deals = this.selected, index = null) {
				...
				vm.loading = true
				this.$axios
				.post('manager/deals/options/archive', {
					deals: deals
				})
				.then(response => {
					if (index !== null) {
			...
			remove: function(deals = this.selected, index = null) {
				...
				vm.loading = true
				this.$axios
				.post('manager/deals/options/delete', {
					deals: deals
				})
				.then(response => {
					if (index !== null) {
