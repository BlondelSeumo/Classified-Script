<template>
	<v-app id="inspire" class="administrator-section">
		...
		<!-- Navbar -->
		<v-app-bar :clipped-right="$vuetify.rtl ? true : false" app :clipped-left="!$vuetify.rtl ? true : false" color="light-blue" dark fixed flat class="admin-header">
			...
			<v-tooltip bottom>
				<template v-slot:activator="{ on }">
					<v-btn text icon color="white" v-on="on" @click.prevent="logout">
						<v-icon>mdi-logout</v-icon>
					</v-btn>
				</template>
				<span>{{ $t('t_sign_out') }}</span>
			</v-tooltip>
		</v-app-bar>
