    ...
    <v-app id="inspire">
		...
		<v-dialog v-model="dialogs.delete" max-width="400" persistent>
			<v-card class="py-2">
				...
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn color="grey darken-1" flat @click="dialogs.delete = false">
						{{ $t('t_cancel') }}
					</v-btn>
			...
			<v-card
				...
				<v-card
					...
					<v-card-title class="pa-2">
						...
						<v-spacer></v-spacer>
						<v-btn icon flat :to="'/shop/' + store.details.shop.store" target="_blank" :color="$vuetify.theme.dark ? 'white' : '#4C4A48'">
							<v-icon>mdi-open-in-new</v-icon>
						</v-btn>
						<v-btn icon flat :to="'/manager/shops/edit/' + store.details.shop.store" :color="$vuetify.theme.dark ? 'white' : '#4C4A48'">
							<v-icon>mdi-square-edit-outline</v-icon>
						</v-btn>
						<v-btn icon flat @click="remove([store.details.shop], store.details.index)" :color="$vuetify.theme.dark ? 'white' : '#4C4A48'" v-if="store.details.shop.deleted_at === null">
							<v-icon>mdi-delete</v-icon>
						</v-btn>
						<v-btn icon flat @click="dialogs.details = false" :color="$vuetify.theme.dark ? 'white' : '#4C4A48'">
		...
		<v-container>
			<div class="m-card">
				<v-toolbar flat>
					<v-toolbar-title>{{ $t('t_my_shops') }}</v-toolbar-title>
					<v-spacer></v-spacer>
			        ...
			        <v-tooltip top>
						<template v-slot:activator="{ on }">
							<v-btn to="/manager/shops/create" v-on="on" flat icon>
								<v-icon>add</v-icon>
							</v-btn>
						</template>
						...
						<template v-slot:item.options="{ item }">
							<v-menu bottom :left="$vuetify.rtl ? false : true" :right="$vuetify.rtl ? true : false" origin="center center" max-height="300px">
								...
								<v-list dense>
									...
									<!-- Edit -->
									<v-list-item :to="'/manager/shops/edit/' + item.store">
										<v-list-item-action>
											<v-icon>mdi-square-edit-outline</v-icon>
										</v-list-item-action>
										<v-list-item-content>
									...
									<!-- Delete -->
									<v-list-item v-if="item.deleted_at === null" @click="remove([item], shops.indexOf(item))">
										<v-list-item-action>
											<v-icon>mdi-delete</v-icon>
										</v-list-item-action>
										<v-list-item-content>
...
<script>
    export default {
		...
		async asyncData({ app, $axios, redirect }) {
			let response = await $axios.get('manager/shops')
			return {
		...
		methods: {
			...
			remove: function(shops = this.selected, index = null) {
				...
				vm.loading = true
				this.$axios
				.post('manager/shops/options/delete', {
					shops: shops
				})
				.then(response => {
					if (index !== null) {
