	...
	<v-app id="inspire">
		...
		<v-container>
			<v-alert
				...
				class="mb-4"
				>
				{{ $t('t_toasted_shops_limit_exceeded') }}
				</v-alert>
			<v-card class="m-card" flat>
				<v-card-title primary-title class="pa-4">
		          <div>
		            <div class="title">{{ $t('t_create_shop') }}</div>
		            <span class="card-description">{{ $t('t_fill_form_create_shop') }}</span>
		          </div>
		        </v-card-title>
					...
					<v-layout wrap>
						...
						<v-flex xs12>
							<div class="form-group form-group-ckeditor" :class="[errors.description ? 'has-danger' : '']">
								...
								<ckeditor :config="{language: this.$i18n.locale}" class="form-control" type="classic" :readonly="exceeded" v-model="form.description"></ckeditor>
							</div>
						...
						<v-flex xs12 class="mb-3">
							<upload-btn uniqueId :disabled="exceeded" @file-update="logo" block accept="image/*" :title="$t('t_choose_logo')" :color="$vuetify.theme.dark ? 'grey darken-4' : 'grey lighten-3'" class="pa-0"></upload-btn>
						</v-flex>

						<!-- Shop cover -->
						<v-flex xs12 class="mb-3">
							<upload-btn uniqueId :disabled="exceeded" @file-update="cover" block accept="image/*" :title="$t('t_choose_cover')" :color="$vuetify.theme.dark ? 'grey darken-4' : 'grey lighten-3'" class="pa-0"></upload-btn>
						</v-flex>

						<v-flex xs12>
							<v-btn @click="create" :disabled="loading || exceeded" :loading="loading" block depressed color="blue" class="mb-1 white--text">{{ $t('t_create') }}</v-btn>
						</v-flex>
...
<script>
	export default {
		...
		async asyncData({ app, $axios, redirect }) {
			let response = await $axios.post('manager/shops/options/create/fetch')
			return {
		...
		methods: {
			create: function() {
				...
				this.$axios
				.post('manager/shops/options/create',
				  	data,
				  	{
					    headers: {
				...
				.then(response => {
					...
					this.$toasted.show(this.$t('t_toasted_shop_created'), {
						icon: 'done_all',
						className: this.$vuetify.rtl ? 'toasted-rtl' : ''
					})
					this.$store.dispatch('manager/setShopsLeft', response.data.left)
