    ...
    <v-app id="inspire">
        <v-container fluid class="pa-0">
            ...
            <v-layout row>
                ...
                <!-- Form -->
                <v-flex xs12 v-if="settings.register">
                    <v-card flat class="hd-fullscreen pa-12 no-border-radius">
                        <v-container grid-list-xl fluid>
                            <v-layout wrap>
                                ...
                                <v-flex  class="mt-5"><v-divider></v-divider></v-flex>

                                <v-flex xs12 class="mt-5 follow-btn">

                                    <!-- Facebook -->
                                    <v-btn v-if="settings.social.isFacebook" block depressed dark small text :color="$vuetify.theme.dark ? '#bbbbbb' : '#3C5A99'" class="px-5 mb-4" @click="$auth.loginWith('facebook')">
                                        {{ $t('t_login_via_facebook') }}
                                    </v-btn>

                                    <!-- Twitter -->
                                    <v-btn v-if="settings.social.isTwitter" block depressed dark small text :color="$vuetify.theme.dark ? '#bbbbbb' : '#1DA1F2'" class="px-5 mb-4" @click.prevent="authenticate('twitter')">
                                        {{ $t('t_login_via_twitter') }}
                                    </v-btn>

                                    <!-- Google -->
                                    <v-btn v-if="settings.social.isGoogle" block depressed dark small text :color="$vuetify.theme.dark ? '#bbbbbb' : '#DC4E41'" class="px-5 mb-4" @click="$auth.loginWith('google')">
                                        {{ $t('t_login_via_google') }}
                                    </v-btn>

                                    <!-- Instagram -->
                                    <v-btn v-if="settings.social.isInstagram" block depressed dark small text :color="$vuetify.theme.dark ? '#bbbbbb' : '#E4405F'" class="px-5 mb-4" @click="authenticate('instagram')">
                                        {{ $t('t_login_via_instagram') }}
                                    </v-btn>

                                    <!-- Pinterest -->
                                    <v-btn v-if="settings.social.isPinterest" block depressed dark small text :color="$vuetify.theme.dark ? '#bbbbbb' : '#BD081C'" class="px-5 mb-4" @click="authenticate('pinterest')">
                                        {{ $t('t_login_via_pinterest') }}
                                    </v-btn>

                                    <!-- Linkedin -->
                                    <v-btn v-if="settings.social.isLinkedin" block depressed dark small text :color="$vuetify.theme.dark ? '#bbbbbb' : '#0077B5'" class="px-5 mb-4" @click="authenticate('linkedin')">
                                        {{ $t('t_login_via_linkedin') }}
                                    </v-btn>

                                    <!-- Vk -->
                                    <v-btn v-if="settings.social.isVk" block depressed dark small text :color="$vuetify.theme.dark ? '#bbbbbb' : '#6383A8'" class="px-5 mb-4" @click="authenticate('vk')">
                                        {{ $t('t_login_via_vk') }}
                                    </v-btn>
                ...
                <v-flex xs12 v-else>
                    <v-card flat class="hd-fullscreen px-12">
                        <div style="position:absolute;top:40%;text-align:center;right:5%;left:5%;" class="px-12">
                            ...
                            <v-btn class="mt-8" block depressed v-t="'t_home'" nuxt to="/"></v-btn>
                            <v-btn class="mt-4" block depressed v-t="'t_login'" nuxt to="/auth/login"></v-btn>
                        </div>
