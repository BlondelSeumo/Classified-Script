			...
			<v-card
				...
				<v-card
					...
					<v-card-title class="pa-2">
						...
						<v-btn icon text :to="'/administrator/shops/edit/' + store.details.shop.store" :color="$vuetify.theme.dark ? 'white' : '#4C4A48'" v-if="$adgate.allow($auth.user, 'edit', 'shops')">
							<v-icon>mdi-square-edit-outline</v-icon>
						</v-btn>
						<v-btn icon text @click="remove([store.details.shop], store.details.index)" :color="$vuetify.theme.dark ? 'white' : '#4C4A48'" v-if="store.details.shop.deleted_at === null && $adgate.allow($auth.user, 'delete', 'shops')">
							<v-icon>mdi-delete</v-icon>
						</v-btn>
						<v-btn icon text @click="restore([store.details.shop], store.details.index)" :color="$vuetify.theme.dark ? 'white' : '#4C4A48'" v-if="store.details.shop.deleted_at === null && $adgate.allow($auth.user, 'delete', 'shops')">
