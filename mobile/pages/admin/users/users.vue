		...
		<v-dialog v-model="dialogs.details" max-width="600" persistent v-if="$adgate.allow($auth.user, 'access', 'users')">
			<v-card
				...
				<v-card
					...
					flat
					>
					<v-card-title class="pa-2 no-border-radius" :class="$vuetify.theme.dark ? 'grey darken-4' : 'blue-grey lighten-5'">
						<v-btn icon @click="dialogs.details = false">
							<v-icon>mdi-close</v-icon>
						</v-btn>
						<h3 class="title font-weight-bold text-uppercase text-center grow">{{ selectedUser.username }}</h3>
						<v-avatar>
			...
			<div class="m-card">
				<v-toolbar flat>
					...
					<v-fade-transition v-if="$adgate.allow($auth.user, 'approve', 'users')">
				        <v-tooltip top v-if="selected.length > 0">
							<template v-slot:activator="{ on }">
								<v-btn v-on="on" text icon :color="$vuetify.theme.dark ? 'white' : 'grey darken-3'" @click="activate()">
			        ...
			        <v-fade-transition v-if="$adgate.allow($auth.user, 'delete', 'users')">
				        <v-tooltip top v-if="selected.length > 0">
							<template v-slot:activator="{ on }">
								<v-btn v-on="on" text icon :color="$vuetify.theme.dark ? 'white' : 'grey darken-3'" @click="dialogs.block = true">
			        ...
			        <v-fade-transition v-if="$adgate.allow($auth.user, 'delete', 'users')">
				        <v-tooltip top v-if="selected.length > 0">
							<template v-slot:activator="{ on }">
								<v-btn v-on="on" text icon :color="$vuetify.theme.dark ? 'white' : 'grey darken-3'" @click="unblock()">
					...
					<v-tooltip top v-if="$adgate.allow($auth.user, 'create', 'users')">
						<template v-slot:activator="{ on }">
							<v-btn to="/administrator/users/create" v-on="on" text icon :color="$vuetify.theme.dark ? 'white' : 'grey darken-3'">
