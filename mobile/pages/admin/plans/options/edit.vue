		...
		<v-container>
			<v-card class="m-card" flat>
				<v-card-title primary-title class="pa-4">
		          <div>
		            <div class="title">{{ $t('t_edit_plan') }}</div>
		            <span class="card-description">{{ $t('t_edit_plan_para') }}</span>
		          </div>
		        </v-card-title>
				...
				<v-container grid-list-xl fluid class="pa-4">
					<v-layout wrap>
						...
						<!-- Upload icon -->
						<v-flex xs12>
							<upload-btn uniqueId @file-update="icon" block accept="image/*" :title="$t('t_choose_icon')" :color="$vuetify.theme.dark ? 'grey darken-4' : 'grey lighten-3'" class="pa-0 mb-3"></upload-btn>
						</v-flex>
...
<script>
	export default {
		...
		async asyncData({ app, $axios, params, redirect }) {
			// Check if allowed
			if (!app.$owgate.allow(app.$auth.user, 'edit', 'plans')) {
				redirect('/administrator')
			}

			let response = await $axios.post('administrator/membership/plans/options/edit', { id: params.id })
			return {
				form: {
					id                            : response.data.plan.unique_id,
					title                         : response.data.plan.title,
					subtitle                      : response.data.plan.subtitle,
					description                   : response.data.plan.description,
					slug                          : response.data.plan.slug,
					price                         : response.data.plan.price,
					discount                      : response.data.plan.discount,
					currency                      : response.data.plan.currency,
					frequency                     : response.data.plan.frequency,
					recommended                   : response.data.plan.isRecommended,
					maximum_stores                : response.data.plan.stores_limit,
					maximum_classifieds           : response.data.plan.classifieds_limit,
					maximum_classifieds_images    : response.data.plan.classifieds_images_limit,
					classifieds_expiration_period : response.data.plan.classifieds_expiry,
					enable_stores                 : response.data.plan.isStores,
					enable_autoshare              : response.data.plan.isAutoshare,
				},
		...
		methods: {
			update: function() {
				// Not allowed?
				if (!this.$owgate.allow(this.$auth.user, 'edit', 'plans')) {
				...
				this.$axios
				.post('administrator/membership/plans/options/update',
				  	data,
				  	{
					    headers: {
				...
				.then(response => {
					this.errors   = []
					this.$toasted.show(this.$t('t_toasted_plan_updated'), {
						icon: 'done_all',
