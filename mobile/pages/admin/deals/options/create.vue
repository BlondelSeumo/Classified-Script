                    ...
                    <v-card class="m-card px-3" flat>
                        ...
                        <v-container grid-list-xl fluid class="pa-4">
                            <v-layout wrap>
                                ...
                                <!-- Update -->
                                <v-flex xs12>
                                    <v-btn @click.prevent="create" :disabled="loading" :loading="loading" block depressed color="blue" class="mb-1" dark>{{ $t('t_create') }}</v-btn>
                                </v-flex>
...
<script>
	...
	export default {
		...
		async asyncData({ app, $axios, params, redirect }) {
			...
			let response = await $axios.post(`administrator/deals/options/create/fetch`)
			return {
				form: {
		...
		methods: {
			create: function() {
				...
				try {
					...
					this.$axios
					.post('administrator/deals/options/create',
					  	data,
					  	{
						    headers: {
