...
<script>
	...
	export default {
		...
		async asyncData({ app, $axios, params, redirect }) {
			// Check if allowed
			if (!app.$adgate.allow(app.$auth.user, 'edit', 'deals')) {
				redirect('/administrator')
			}

			let response = await $axios.get(`administrator/deals/options/edit/${params.id}`)
			return {
				form: {
		...
		methods: {
			update() {
                try {
					...
					this.$axios
					.post('administrator/deals/options/edit',
					  	data,
					  	{
						    headers: {
					...
					.then(response => {
						...
						this.$router.push('/administrator/deals')
						this.loading = false
					})
					.catch(error => {
