			...
			<div class="m-card">
				<v-toolbar flat>
			        ...
			        <v-fade-transition v-if="$owgate.allow($auth.user, 'access', 'subscriptions')">
				        <v-tooltip top v-if="selected.length > 0">
							<template v-slot:activator="{ on }">
								<v-btn v-on="on" text icon :color="$vuetify.theme.dark ? 'white' : 'grey darken-3'" @click="accept()">
			        ...
			        <v-fade-transition v-if="$owgate.allow($auth.user, 'access', 'subscriptions')">
				        <v-tooltip top v-if="selected.length > 0">
							<template v-slot:activator="{ on }">
								<v-btn v-on="on" text icon :color="$vuetify.theme.dark ? 'white' : 'grey darken-3'" @click="decline()">
