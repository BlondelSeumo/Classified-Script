		...
		<v-container>
			<v-card class="m-card" flat>
				<v-card-title primary-title class="pa-4">
		          <div>
		            <div class="title">{{ $t('t_create_category') }}</div>
		            <span class="card-description">{{ $t('t_create_category_paragraph') }}</span>
		          </div>
		        </v-card-title>
				...
				<v-container grid-list-xl fluid class="pa-4">
					<v-layout wrap>
						...
						<!-- Upload icon -->
						<v-flex xs12>
							<upload-btn uniqueId @file-update="icon" block accept="image/*" :title="$t('t_choose_icon')" :color="$vuetify.theme.dark ? 'grey darken-4' : 'grey lighten-3'" class="pa-0"></upload-btn>
						</v-flex>
...
<script>
	export default {
		...
		async asyncData({ app, $axios, redirect }) {
			// Check if allowed
			if (!app.$adgate.allow(app.$auth.user, 'create', 'categories')) {
				redirect('/administrator')
			}

			let response = await $axios.post('administrator/ajax/fetch/categories')
			return {
		...
		methods: {
			create: function() {
				// Not allowed?
				if (!this.$adgate.allow(this.$auth.user, 'create', 'categories')) {
				...
				this.$axios
				.post('administrator/categories/options/create',
				  	data,
				  	{
					    headers: {
				...
				.then(response => {
					...
					this.selected       = ''
					this.$toasted.show(this.$t('t_toasted_category_created'), {
						icon: 'done_all',
