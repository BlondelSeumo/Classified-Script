				...
				<v-container grid-list-xl fluid class="pa-4">
					<v-layout wrap>
						...
						<v-flex xs12 class="mb-3">
							<upload-btn uniqueId @file-update="logo" block accept="image/*" :title="$t('t_change_logo')" :color="$vuetify.theme.dark ? 'grey darken-4' : 'grey lighten-3'" class="pa-0"></upload-btn>
						</v-flex>

						<!-- Shop cover -->
						<v-flex xs12 class="mb-3">
							<upload-btn uniqueId @file-update="cover" block accept="image/*" :title="$t('t_change_cover')" :color="$vuetify.theme.dark ? 'grey darken-4' : 'grey lighten-3'" class="pa-0"></upload-btn>
						</v-flex>
...
<script>
	export default {
		...
		async asyncData({ app, $axios, params, redirect }) {
			// Check if allowed
			if (!app.$mogate.allow(app.$auth.user, 'edit', 'shops')) {
				redirect('/moderator')
			}

			let response = await $axios.post('moderator/shops/options/edit', { shop: params.shop })
			return {
				form: {
		...
		methods: {
			update: function() {
				// Check if allowed
				if (!this.$mogate.allow(this.$auth.user, 'edit', 'shops')) {
					return
				}
				...
				this.$axios
				.post('moderator/shops/options/update',
				  	data,
				  	{
					    headers: {
				...
				.then(response => {
					...
					if (this.form.old !== response.data) {
						this.$router.push({ name: 'moderatorEditShop', params: { shop: response.data }})
					}
				})
				.catch(error => {
