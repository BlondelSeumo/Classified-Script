...
<script>
	...
	export default {
		...
		async asyncData({ app, $axios, params, redirect }) {
			// Check if allowed
			if (!app.$mogate.allow(app.$auth.user, 'edit', 'deals')) {
				redirect('/moderator')
			}

			let response = await $axios.get(`moderator/deals/options/edit/${params.id}`)
			return {
				form: {
		...
		methods: {
			update() {
                try {
					...
					this.$axios
					.post('moderator/deals/options/update',
					  	data,
					  	{
						    headers: {
					...
					.then(response => {
						...
						this.$router.push('/moderator/deals')
						this.loading = false
					})
					.catch(error => {
